...
class TestRequests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        #Create user request
        response = self.client().post('/api/v2/users/requests', data = json.dumps(self.request),
    ...
    def test_api_for_user_read_all_request(self):
        ...
        response = self.client().get("/api/v2/users/requests", headers=self.headers)
        print(self.headers['token'])
        self.assertEquals(response.status_code, 200)

    def test_api_to_view_a_request(self):
        #test api to view request
        response = self.client().get("/api/v2/users/requests/1", headers=self.headers)
        print(self.headers['token'])
        self.assertEquals(response.status_code, 200)
    ...
    def test_api_to_delete_a_request(self):
        ...
        response = self.client().delete('/api/v2/users/requests/1', headers=self.headers)
        self.assertEquals(response.status_code, 200)
